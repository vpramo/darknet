...
std::string encode(const Mat & frame)
{
    ...
    auto *enc_msg = reinterpret_cast<unsigned char*>(outbuf.data());
    std::string encoded = base64_encode(enc_msg, outbuf.size());
    return encoded;
}
...
void send_json(detection *dets, int nboxes, int classes, char **names, long long int frame_id, int port, int timeout, mat_cv* base64_img)
{
    try {
        ...
        std::string str= encode(*(cv::Mat*)base64_img);
        char *cstr = &str[0];
        char *send_buf = detection_to_json(dets, nboxes, classes, names, frame_id, NULL, cstr);
